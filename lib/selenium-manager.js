 ...
 * @type {Promise}
 */
var DEFAULT_PHANTOM_JS_DRIVER = os.platform().indexOf('win') >= 0 ? path.resolve(__dirname, '../node_modules/phantomjs/bin/phantomjs') : path.resolve(__dirname, '../node_modules/phantomjs/phantomjs.exe');
...
SeleniumManager.prototype.start = function (callback, option) {
	...
	async.map([serverJar, ieDriver, phantomDriver], fs.stat, function (err) {
		...
		this._command = spawn(this._javaInterpreter, ['-jar', serverJar, '-Dwebdriver.ie.driver=', '"' + ieDriver + '"', '-Dwebdriver.phantomjs.driver=', '"' + phantomDriver + '"', '-port', port]);
